...
class MobiusPipeline:
    ...
    def __init__(self,
        ...
        self.n_multi_fwd = n_multi_fwd
        torchmobius.attribute.FORWARD_MICROBTAH_NUM = len(physic_devices)
        torchmobius.attribute.BACKWARD_MICROBATCH_NUM = torchmobius.attribute.FORWARD_MICROBTAH_NUM
        # torchgpipe.attribute.BACKWARD_MICROBATCH_NUM = len(physic_devices)
    ...
    def run(self) -> None:
        ...
        with spawn_workers(physic_devices) as (in_queues, out_queues):
    ...
    def compute(self,
        ...
        for i, j in schedule:
            ...
            if j != 0:
                wait(batch, copy_streams[physic_device_index][i], streams[physic_device_index])
            ...
            if checkpoint:
                ...
                task = Task(streams[physic_device_index], compute=chk.checkpoint, finalize=chk.recompute)
                    
                    # Compute tasks in parallel. ([2] in the diagram)
                in_queues[physic_device_index].put(task)
        ...
        for i, j in schedule:
            ...
            ok, payload = out_queues[physic_device_index].get()
            ...
            # diagram)
            if j != n-1:
                wait(batch, streams[physic_device_index], copy_streams[physic_device_index][i])
