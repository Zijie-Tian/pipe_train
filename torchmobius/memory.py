# https://github.com/microsoft/DeepSpeed/tree/master/deepspeed/runtime/zero/contiguous_memory_allocator.py
...
class ContiguousMemoryAllocator(object):
    ...
    def allocate_tensor(self, size):
        with self.mutex:
            return self._allocate_tensor(size)
            
    def _allocate_tensor(self, size):
    ...
    def assign_to_param(self, tensor, param, numel, shape):
        with self.mutex:
            self._assign_to_param(tensor, param, numel, shape)
            
    def _assign_to_param(self, tensor, param, numel, shape):
        tensor_id = id(tensor)
        ...
        assert not tensor_id in self.id_to_params.keys(), f"This tensor has already been assigned to a param, {tensor_id}, {id(self.id_to_params[tensor_id][0])}, {id(param)}"
    ...
    def release_tensor(self, tensor):
        with self.mutex:
            self._release_tensor_f(tensor)
            
    def _release_tensor_f(self, tensor):
        free_before = self.total_free
        tensor_id = id(tensor)
    ...
    def release_tensor_with_id(self, tensor_id):
        with self.mutex:
            self._release_tensor_with_id(tensor_id)
    
    def _release_tensor_with_id(self, tensor_id):
        free_before = self.total_free
