...
def add_flops_counting_methods(net_main_module, custom_modules_hooks={}):
...
def start_flops_count(self, **kwargs):
    ...
    def add_flops_counter_hook_function(module, ost, verbose, ignore_list):
        if type(module) in ignore_list:
            ...
            elif type(module).__name__ in CUSTOM_MODULES_MAPPING:
                handle = module.register_forward_hook(
                                        CUSTOM_MODULES_MAPPING[type(module).__name__])
            else:
                handle = module.register_forward_hook(MODULES_MAPPING[type(module)])
                
            # print('INFO: module ' + type(module).__name__ +
            #           ' is registered.', file=ost)
...
def is_supported_instance(module):
    if type(module) in MODULES_MAPPING or \
        type(module) in CUSTOM_MODULES_MAPPING or \
        type(module).__name__ in  CUSTOM_MODULES_MAPPING   :
        return True
    return False
